...
export function meta() {
  return [
    { title: "Danh sách ngành học - VMU Training" },
...
export default function MajorsPage() {
  ...
  // ============================================
  const fetchMajorsData = async (params: TableQueryParams): Promise<PaginatedResponse<Major>> => {
    try {
      const allData = await majorService.getDanhSach();

      // Client-side filtering
      let filtered = allData;
      if (params.search) {
        const keyword = params.search.toLowerCase().trim();
        filtered = allData.filter(major =>
          major.ma?.toLowerCase().includes(keyword) ||
          major.tenNganhHoc?.toLowerCase().includes(keyword)
        );
      }
        ...
        <div className="text-center">
          {item.daoTaoThacSy ? (
            <Badge variant="success">
              {item.thoiGianDaoTaoThacSy ? `${item.thoiGianDaoTaoThacSy} năm` : 'Có'}
            </Badge>
          ) : (
            <Badge variant="default">Không</Badge>
          )}
        </div>
      ),
    },
    {
      ...
      render: (item: Major) => (
        <div className="text-center">
          {item.daoTaoTienSy ? (
            <Badge variant="info">
              {item.thoiGianDaoTaoTienSy ? `${item.thoiGianDaoTaoTienSy} năm` : 'Có'}
            </Badge>
          ) : (
            <Badge variant="default">Không</Badge>
          )}
    ...
    },
    {
      ...
      render: (item: Major) => (
        <div className="text-center text-sm text-gray-600">
          {item.daoTaoThacSy && item.thoiGianDaoTaoThacSy ? (
            <div className="text-xs">
              <div>ThS: {item.thoiGianDaoTaoThacSy} năm</div>
              {item.thoiGianVuotKhungThacSy && (
                <div className="text-gray-500">Tối đa: {item.thoiGianVuotKhungThacSy} năm</div>
              )}
            </div>
          ) : item.daoTaoTienSy && item.thoiGianDaoTaoTienSy ? (
            <div className="text-xs">
              <div>TS: {item.thoiGianDaoTaoTienSy} năm</div>
              {item.thoiGianVuotKhungTienSy && (
                <div className="text-gray-500">Tối đa: {item.thoiGianVuotKhungTienSy} năm</div>
              )}
            </div>
          ) : (
            '-'
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Danh sách ngành học</h1>
      ...
      <Card>
        <div className="p-4 space-y-4">
          ...
          <Table
            ...
            emptyMessage={
              searchForm.values.search
                ? `Không tìm thấy ngành học nào với từ khóa "${searchForm.values.search}"`
